...
def _resolve_runtime_asset_path(path_text: str) -> Path | None:
    relative = str(path_text or "").strip()
    if not relative:
        return None
    candidate = Path(os.path.expanduser(relative))
    if candidate.is_absolute():
        return candidate.resolve(strict=False) if candidate.exists() else None
    for root in (
...
def _resolve_bundled_emcc_wrapper(runtime_config: dict[str, Any]) -> str:
    toolchains = runtime_config.get("toolchains", {}) if isinstance(runtime_config.get("toolchains", {}), dict) else {}
    emscripten = toolchains.get("emscripten", {}) if isinstance(toolchains.get("emscripten", {}), dict) else {}
    candidates = [
        str(emscripten.get("emcc_wrapper") or "").strip(),
        str(runtime_config.get("bundled_emcc") or "").strip(),
        "toolchains/nova-emcc.bat",
    ]
    for candidate in candidates:
        resolved = _resolve_runtime_asset_path(candidate)
...
def resolve_emcc_command(runtime_config: dict[str, Any], modules: dict[str, bool]) -> str:
    bundled = _resolve_bundled_emcc_wrapper(runtime_config)
    if bundled:
        return bundled
...
@dataclass
class NovaShellCommandExecutor:
    shell: "NovaShell"

    def execute(self, command: str, *, pipeline_data: Any = None, cwd: Path | None = None) -> DeclarativeCommandExecution:
        previous_cwd = self.shell.cwd
        if cwd is not None:
            self.shell.cwd = cwd
        try:
            if pipeline_data is None:
                ...
                result = self.shell._route_with_input(command, pipeline_data)
        finally:
            if cwd is not None:
                self.shell.cwd = previous_cwd
        return DeclarativeCommandExecution(
            output=result.output,
            data=result.data,
            error=result.error,
            metadata={"data_type": result.data_type.value},
        )
    ...
    def compile_to_wasm_and_run(self, code: str, pipeline_input: str = "") -> CommandResult:
        ...
        try:
            ...
            try:
                ...
                return CommandResult(
                    output=stdout_text or "sandbox executed\n",
...
class MeshScheduler:
    ...
    def list_workers(self, *, include_secrets: bool = False) -> list[dict[str, Any]]:
        ...
        for w in sorted(self.workers, key=lambda item: (item["load"], item["url"])):
            row = {
                ...
                "tenant": w.get("tenant"),
                "tls_profile": w.get("tls_profile"),
                "draining": bool(w.get("draining", False)),
            }
            for key in ("managed_local", "worker_id", "pid", "log_path"):
                if key in w:
                    row[key] = w[key]
            if include_secrets and "auth_token" in w:
                row["auth_token"] = w.get("auth_token")
            rows.append(row)
        return rows
...
class NovaAtheriaRuntime:
    ...
    def _ensure_core(self) -> Any:
        ...
        try:
            ...
            self._core_init_error = str(exc).strip() or exc.__class__.__name__
            raise
    ...
    def status_payload(self) -> dict[str, Any]:
        ...
        if self._core_init_error:
            payload["core_error"] = self._core_init_error
        if self._core is not None:
            with contextlib.suppress(Exception):
...
class MeshWorkerServer:
    ...
    def serve(self, host: str, port: int) -> int:
        ...
        def protocol_command(task: ExecutorTask) -> str | None:
            if task.command:
                return task.command
            operation = str(task.operation or "").strip()
            ...
            rendered = " ".join(json.dumps(item, ensure_ascii=False) for item in arguments)
            return f"{operation} {rendered}".strip()
            ...
            def do_POST(self) -> None:  # noqa: N802
                ...
                if auth_token:
                    ...
                    if auth_header != f"Bearer {auth_token}":
                        self._write_json({"output": "", "data": None, "error": "unauthorized", "data_type": PipelineType.TEXT.value}, status=401)
                        return
                length = int(self.headers.get("Content-Length", "0"))
                body = self.rfile.read(length) if length > 0 else b"{}"
                try:
                ...
                if parsed.path == "/protocol/execute":
                    ...
                    if not command:
                        ...
                        self._write_json(response.to_dict(), status=400)
                        return
                    result = shell._route_with_input(command, task.pipeline_data) if task.pipeline_data is not None else shell.route(command)
                    response = ExecutorResult(
                        request_id=task.request_id,
                        ...
                        metadata={"worker_id": shell.node_id, "caps": caps},
                    )
                    self._write_json(response.to_dict(), status=200 if result.error is None else 500)
                    return
                ...
                if not command:
                    ...
                    return
                result = shell._route_with_input(command, pipeline_data) if pipeline_data is not None else shell.route(command)
                response = {
                    "output": result.output,
                    "data": result.data,
                    "error": result.error,
...
class NovaShell:
    def __init__(self) -> None:
        ...
        self.commands: dict[str, Callable[[str, str, Any], CommandResult]] = {
            ...
            "ns.graph": self._ns_graph,
            "ns.snapshot": self._ns_snapshot,
            "ns.resume": self._ns_resume,
            "ns.status": self._ns_status,
            "ns.cluster": self._ns_cluster,
            "ns.auth": self._ns_auth,
            "ns.deploy": self._ns_deploy,
    ...
    def _is_declarative_nova_source(self, source: str) -> bool:
        for raw in source.splitlines():
            line = raw.strip()
            if not line or line.startswith("#"):
                continue
            return bool(re.match(r"^(agent|dataset|flow|state|event|tool|system)\s+[A-Za-z_][A-Za-z0-9_.-]*\s*\{", line))
        return False

    def _bridge_declarative_event(self, event: DeclarativeEvent) -> None:
        ...
        if isinstance(payload, str):
            ...
            data_type = PipelineType.OBJECT.value
        self.events.emit(
            {
                "stage": f"nova {event.name}",
                "node": f"nova.{event.name}",
                "error": "",
                "output": preview[:200],
                "data_type": data_type,
                "trace_id": self.current_trace_id,
                "duration_ms": "0.000",
                "rows_processed": "1" if payload is not None else "0",
                "cpu_percent": "0.0",
                "rss_mb": "0.0",
                "cost_estimate": "0.0",
            }
    ...
    def _run_declarative_nova_source(self, source: str, *, source_name: str, base_path: Path, graph_only: bool = False) -> CommandResult:
        try:
            ...
            program = runtime.compile(source, source_name=source_name, base_path=base_path)
            self._ns_runtime = None

            if graph_only:
                payload = {
                    "source_name": source_name,
                    "base_path": str(base_path),
                    "declarations": len(program.ast.declarations),
                    "graph": program.graph.to_dict(),
                }
            else:
    ...
    def _register_declarative_mesh_workers(self, runtime: DeclarativeNovaRuntime) -> None:
        if runtime.context is None:
            return
        for worker in self.mesh.list_workers(include_secrets=True):
            worker_id = str(worker.get("worker_id") or worker.get("url") or uuid.uuid4().hex[:8])
            caps = {str(cap) for cap in worker.get("caps", [])}
            if not caps:
                continue
            labels = {str(key): str(value) for key, value in dict(worker.get("labels") or {}).items()}
            runtime.context.mesh.register(
                WorkerNode(
                    worker_id=worker_id,
                    capabilities=caps,
                    endpoint=str(worker.get("url") or ""),
                    auth_token=str(worker.get("auth_token")) if worker.get("auth_token") else None,
                    tls_profile=str(worker.get("tls_profile")) if worker.get("tls_profile") else None,
                    tenant=str(worker.get("tenant")) if worker.get("tenant") else None,
                    draining=bool(worker.get("draining", False)),
                    labels={
                        **labels,
                        "managed_local": str(bool(worker.get("managed_local"))).lower(),
                    },
                )
            )
    ...
    def _active_declarative_runtime(self) -> DeclarativeNovaRuntime:
        if self._declarative_nova is None or self._declarative_nova.context is None:
            raise RuntimeError("no active declarative Nova runtime")
        return self._declarative_nova

    def _allows_bootstrap_admin(self, runtime: DeclarativeNovaRuntime) -> bool:
        snapshot = runtime.context.security.snapshot()
        return int(snapshot.get("active_token_count") or 0) == 0

    def _require_declarative_access(self, action: str, *, admin: bool = False, allow_bootstrap: bool = False) -> DeclarativeNovaRuntime:
        runtime = self._active_declarative_runtime()
        if admin:
            if allow_bootstrap and self._allows_bootstrap_admin(runtime):
                return runtime
            runtime.assert_admin_access(action)
        else:
            runtime.assert_operator_access(action)
        return runtime

    def _json_command_result(self, payload: Any) -> CommandResult:
        return CommandResult(output=json.dumps(payload, ensure_ascii=False) + "\n", data=payload, data_type=PipelineType.OBJECT)

    def _parse_json_argument(self, value: str) -> Any:
        payload = value.strip()
        if not payload:
            return {}
    ...
    def _parse_key_value_pairs(self, value: str) -> dict[str, str]:
        items: dict[str, str] = {}
    ...
    def _ns_exec(self, script: str, _: str, __: Any) -> CommandResult:
        ...
        if self._is_declarative_nova_source(source):
            return self._run_declarative_nova_source(source, source_name="<inline>", base_path=self.cwd)
        try:
    ...
    def _ns_run(self, file_path: str, _: str, __: Any) -> CommandResult:
        ...
        try:
            ...
            if self._is_declarative_nova_source(source):
                return self._run_declarative_nova_source(source, source_name=str(target), base_path=target.parent)

            parser = LegacyNovaParser()
            interpreter = NovaInterpreter(self)
            nodes = parser.parse_file(str(target))
            self._ns_runtime = interpreter
            self._declarative_nova = None
            output = interpreter.execute(nodes)
            return CommandResult(output=output)
    ...
    def _ns_graph(self, payload: str, _: str, __: Any) -> CommandResult:
        parts = split_command(payload)
        if not parts:
            return CommandResult(output="", error="usage: ns.graph <script.ns>")
        try:
            path = self._resolve_path(parts[0])
            source = path.read_text(encoding="utf-8")
            if not self._is_declarative_nova_source(source):
                return CommandResult(output="", error="ns.graph only supports declarative Nova programs")
    ...
    def _ns_status(self, _: str, __: str, ___: Any) -> CommandResult:
        if self._declarative_nova is None or self._declarative_nova.context is None or self._declarative_nova.program is None:
            return CommandResult(output="", error="no active declarative Nova runtime")
        payload = {
            "source_name": self._declarative_nova.program.source_name,
            "graph": self._declarative_nova.program.graph.to_dict(),
            "context": self._declarative_nova.context.snapshot(),
        }
        return CommandResult(output=json.dumps(payload, ensure_ascii=False) + "\n", data=payload, data_type=PipelineType.OBJECT)

    def _ns_cluster(self, payload: str, _: str, __: Any) -> CommandResult:
        try:
            runtime = self._require_declarative_access("ns.cluster", admin=False)
            parts = split_command(payload)
            if not parts or parts[0] == "status":
                cluster_name = parts[1] if len(parts) > 1 else None
                return self._json_command_result({"leader": runtime.leader_status(cluster_name), "cluster": runtime.context.cluster.snapshot(), "mesh_health": runtime.context.mesh.health_report()})

            if parts[0] == "health":
                return self._json_command_result({"mesh_health": runtime.context.mesh.health_report(), "cluster": runtime.context.cluster.snapshot()})

            if parts[0] != "leader":
                return CommandResult(output="", error="usage: ns.cluster [status [cluster]] | ns.cluster health | ns.cluster leader <status|acquire|renew|release> ...")

            if len(parts) == 1 or parts[1] == "status":
                cluster_name = parts[2] if len(parts) > 2 else None
                return self._json_command_result({"leader": runtime.leader_status(cluster_name)})
            runtime = self._require_declarative_access("ns.cluster.leader", admin=True)
            if parts[1] == "acquire":
                cluster_name = parts[2] if len(parts) > 2 else runtime.context.cluster_name
                node_id = parts[3] if len(parts) > 3 else runtime.context.node_id
                lease_seconds = int(parts[4]) if len(parts) > 4 else 30
                return self._json_command_result(runtime.acquire_leadership(cluster_name, node_id=node_id, lease_seconds=lease_seconds))
            if parts[1] == "renew":
                cluster_name = parts[2] if len(parts) > 2 else runtime.context.cluster_name
                node_id = parts[3] if len(parts) > 3 else runtime.context.node_id
                lease_seconds = int(parts[4]) if len(parts) > 4 else 30
                return self._json_command_result(runtime.renew_leadership(cluster_name, node_id=node_id, lease_seconds=lease_seconds))
            if parts[1] == "release":
                cluster_name = parts[2] if len(parts) > 2 else runtime.context.cluster_name
                node_id = parts[3] if len(parts) > 3 else runtime.context.node_id
                return self._json_command_result(runtime.release_leadership(cluster_name, node_id=node_id))
            return CommandResult(output="", error="usage: ns.cluster leader <status|acquire|renew|release> ...")
        except Exception as exc:
            return CommandResult(output="", error=str(exc))

    def _ns_auth(self, payload: str, _: str, __: Any) -> CommandResult:
        try:
            runtime = self._active_declarative_runtime()
            parts = split_command(payload)
            if not parts or parts[0] == "status":
                return self._json_command_result(
                    {
                        "tenant": runtime.context.active_tenant,
                        "namespace": runtime.context.active_namespace,
                        "principal": runtime.whoami(),
                        "security": runtime.context.security.snapshot(),
                    }
                )

            if parts[0] == "whoami":
                return self._json_command_result(runtime.whoami())
            if parts[0] == "login":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.auth login <token>")
                return self._json_command_result(runtime.login(parts[1]))
            if parts[0] == "logout":
                return self._json_command_result(runtime.logout())

            if parts[0] == "tenant":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.auth tenant <list|create|select> ...")
                if parts[1] == "list":
                    return self._json_command_result({"tenants": runtime.context.security.list_tenants(), "active_tenant": runtime.context.active_tenant})
                if parts[1] == "create":
                    if len(parts) < 3:
                        return CommandResult(output="", error="usage: ns.auth tenant create <tenant_id> [display_name]")
                    runtime = self._require_declarative_access("ns.auth.tenant.create", admin=True, allow_bootstrap=True)
                    display_name = parts[3] if len(parts) > 3 else parts[2]
                    return self._json_command_result(runtime.register_tenant(parts[2], display_name=display_name))
                if parts[1] == "select":
                    if len(parts) < 3:
                        return CommandResult(output="", error="usage: ns.auth tenant select <tenant_id>")
                    runtime = self._require_declarative_access("ns.auth.tenant.select", admin=True, allow_bootstrap=True)
                    return self._json_command_result(runtime.select_tenant(parts[2]))
                return CommandResult(output="", error="usage: ns.auth tenant <list|create|select> ...")

            if parts[0] == "namespace":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.auth namespace <list|select> ...")
                if parts[1] == "list":
                    return self._json_command_result(
                        {
                            "active_namespace": runtime.context.active_namespace,
                            "namespaces": sorted(
                                {
                                    str(item.get("namespace") or runtime.context.active_namespace)
                                    for item in runtime.list_state(limit=1000)
                                }
                                | {runtime.context.active_namespace}
                            ),
                        }
                    )
                if parts[1] == "select":
                    if len(parts) < 3:
                        return CommandResult(output="", error="usage: ns.auth namespace select <namespace>")
                    runtime = self._require_declarative_access("ns.auth.namespace.select", admin=True, allow_bootstrap=True)
                    return self._json_command_result(runtime.select_namespace(parts[2]))
                return CommandResult(output="", error="usage: ns.auth namespace <list|select> ...")

            if parts[0] == "token":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.auth token <issue|verify|revoke> ...")
                if parts[1] == "issue":
                    if len(parts) < 4:
                        return CommandResult(output="", error="usage: ns.auth token issue <tenant_id> <subject> [roles_csv] [ttl_seconds]")
                    runtime = self._require_declarative_access("ns.auth.token.issue", admin=True, allow_bootstrap=True)
                    roles = [role for role in (parts[4].split(",") if len(parts) > 4 else []) if role]
                    ttl_seconds = int(parts[5]) if len(parts) > 5 else 3600
                    return self._json_command_result(runtime.issue_token(parts[2], parts[3], roles=roles, ttl_seconds=ttl_seconds))
                if parts[1] == "verify":
                    if len(parts) < 3:
                        return CommandResult(output="", error="usage: ns.auth token verify <token>")
                    return self._json_command_result(runtime.verify_token(parts[2]))
                if parts[1] == "revoke":
                    if len(parts) < 3:
                        return CommandResult(output="", error="usage: ns.auth token revoke <token_id>")
                    runtime = self._require_declarative_access("ns.auth.token.revoke", admin=True)
                    return self._json_command_result(runtime.revoke_token(parts[2]))
                return CommandResult(output="", error="usage: ns.auth token <issue|verify|revoke> ...")

            if parts[0] == "secret":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.auth secret <list|set|get> ...")
                if parts[1] == "list":
                    runtime = self._require_declarative_access("ns.auth.secret.list", admin=True)
                    tenant_id = parts[2] if len(parts) > 2 else runtime.context.active_tenant
                    return self._json_command_result({"secrets": runtime.context.security.list_secrets(tenant_id)})
                if parts[1] == "set":
                    if len(parts) < 5:
                        return CommandResult(output="", error="usage: ns.auth secret set <tenant_id> <name> <value>")
                    runtime = self._require_declarative_access("ns.auth.secret.set", admin=True)
                    return self._json_command_result(runtime.store_secret(parts[2], parts[3], parts[4]))
                if parts[1] == "get":
                    if len(parts) < 4:
                        return CommandResult(output="", error="usage: ns.auth secret get <tenant_id> <name>")
                    runtime = self._require_declarative_access("ns.auth.secret.get", admin=True)
                    secret = runtime.resolve_secret(parts[2], parts[3])
                    if secret is None:
                        return CommandResult(output="", error=f"secret '{parts[3]}' not found for tenant '{parts[2]}'")
                    return self._json_command_result(secret)
                return CommandResult(output="", error="usage: ns.auth secret <list|set|get> ...")

            if parts[0] == "tls":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.auth tls <list|set> ...")
                if parts[1] == "list":
                    runtime = self._require_declarative_access("ns.auth.tls.list", admin=True)
                    return self._json_command_result({"tls_profiles": runtime.context.security.list_tls_profiles()})
                if parts[1] == "set":
                    if len(parts) < 5:
                        return CommandResult(output="", error="usage: ns.auth tls set <profile> <cert> <key> [ca]")
                    runtime = self._require_declarative_access("ns.auth.tls.set", admin=True)
                    cafile = parts[5] if len(parts) > 5 else None
                    return self._json_command_result(runtime.set_tls_profile(parts[2], parts[3], parts[4], cafile=cafile))
                return CommandResult(output="", error="usage: ns.auth tls <list|set> ...")

            if parts[0] == "trust":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.auth trust <list|set> ...")
                if parts[1] == "list":
                    runtime = self._require_declarative_access("ns.auth.trust.list", admin=True)
                    return self._json_command_result({"trust_policies": runtime.context.security.list_trust_policies()})
                if parts[1] == "set":
                    if len(parts) < 3:
                        return CommandResult(output="", error="usage: ns.auth trust set <name> [tenant] [namespace] [require_tls] [labels_json] [caps_csv]")
                    runtime = self._require_declarative_access("ns.auth.trust.set", admin=True)
                    tenant_id = parts[3] if len(parts) > 3 and parts[3] != "-" else None
                    namespace = parts[4] if len(parts) > 4 and parts[4] != "-" else None
                    require_tls = parts[5].lower() == "true" if len(parts) > 5 else False
                    labels = self._parse_json_argument(parts[6]) if len(parts) > 6 else {}
                    if not isinstance(labels, dict):
                        labels = {}
                    capabilities = [item for item in (parts[7].split(",") if len(parts) > 7 else []) if item]
                    return self._json_command_result(
            ...
            if parts[0] == "worker":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.auth worker <list|onboard|rotate> ...")
                if parts[1] == "list":
                    runtime = self._require_declarative_access("ns.auth.worker.list", admin=True)
                    tenant_id = parts[2] if len(parts) > 2 else None
                    return self._json_command_result({"workers": runtime.context.security.list_worker_enrollments(tenant_id)})
                if parts[1] == "onboard":
                    if len(parts) < 6:
                        return CommandResult(output="", error="usage: ns.auth worker onboard <worker_id> <tenant> <namespace> <caps_csv> <labels_json> [cert] [key] [ca] [trust_policy]")
                    runtime = self._require_declarative_access("ns.auth.worker.onboard", admin=True)
                    labels = self._parse_json_argument(parts[6])
                    if not isinstance(labels, dict):
                        return CommandResult(output="", error="labels_json must be a JSON object")
                    certfile = parts[7] if len(parts) > 7 and parts[7] != "-" else None
                    keyfile = parts[8] if len(parts) > 8 and parts[8] != "-" else None
                    cafile = parts[9] if len(parts) > 9 and parts[9] != "-" else None
                    trust_policy = parts[10] if len(parts) > 10 and parts[10] != "-" else None
                    capabilities = [item for item in parts[5].split(",") if item]
                    return self._json_command_result(
                        runtime.onboard_worker(
                            parts[2],
                            parts[3],
                            namespace=parts[4],
                            capabilities=capabilities,
                ...
                if parts[1] == "rotate":
                    if len(parts) < 5:
                        return CommandResult(output="", error="usage: ns.auth worker rotate <worker_id> <cert> <key> [ca]")
                    runtime = self._require_declarative_access("ns.auth.worker.rotate", admin=True)
                    cafile = parts[5] if len(parts) > 5 else None
                    return self._json_command_result(runtime.rotate_worker_certificate(parts[2], parts[3], parts[4], cafile=cafile))
                return CommandResult(output="", error="usage: ns.auth worker <list|onboard|rotate> ...")

            return CommandResult(output="", error="usage: ns.auth [status|whoami|login|logout] | ns.auth tenant ... | ns.auth namespace ... | ns.auth token ... | ns.auth secret ... | ns.auth tls ... | ns.auth trust ... | ns.auth worker ...")
        except Exception as exc:
            return CommandResult(output="", error=str(exc))

    def _ns_deploy(self, payload: str, _: str, __: Any) -> CommandResult:
        try:
            runtime = self._require_declarative_access("ns.deploy", admin=False)
            parts = split_command(payload)
            if not parts or parts[0] == "status":
                deployment_name = parts[1] if len(parts) > 1 else None
                return self._json_command_result(runtime.deployment_status(deployment_name))

            if parts[0] == "health":
                if len(parts) < 5:
                    return CommandResult(output="", error="usage: ns.deploy health <name> <revision> <target> <healthy|unhealthy> [metrics_json_or_path]")
                runtime = self._require_declarative_access("ns.deploy.health", admin=True)
                metrics = self._parse_json_argument(parts[5]) if len(parts) > 5 else {}
                if not isinstance(metrics, dict):
                    metrics = {"value": metrics}
                return self._json_command_result(runtime.record_deployment_health(parts[1], int(parts[2]), parts[3], status=parts[4], metrics=metrics))
            if parts[0] == "evaluate":
                if len(parts) < 3:
                    return CommandResult(output="", error="usage: ns.deploy evaluate <name> <revision> [minimum_healthy_targets] [max_error_rate]")
                runtime = self._require_declarative_access("ns.deploy.evaluate", admin=True)
                minimum_healthy_targets = int(parts[3]) if len(parts) > 3 else 1
                max_error_rate = float(parts[4]) if len(parts) > 4 else 0.2
                return self._json_command_result(runtime.evaluate_rollout(parts[1], int(parts[2]), minimum_healthy_targets=minimum_healthy_targets, max_error_rate=max_error_rate))

            runtime = self._require_declarative_access("ns.deploy.mutate", admin=True)
            if parts[0] == "rollout":
                if len(parts) < 3:
                    return CommandResult(output="", error="usage: ns.deploy rollout <name> <spec_json_or_path> [strategy] [auto_promote]")
                strategy = parts[3] if len(parts) > 3 else "rolling"
                auto_promote = parts[4].lower() != "false" if len(parts) > 4 else True
                spec = self._parse_json_argument(parts[2])
                if not isinstance(spec, dict):
                    spec = {"value": spec}
                return self._json_command_result(runtime.create_rollout(parts[1], spec, strategy=strategy, auto_promote=auto_promote))
            if parts[0] == "promote":
                if len(parts) < 3:
                    return CommandResult(output="", error="usage: ns.deploy promote <name> <revision>")
                return self._json_command_result(runtime.promote_revision(parts[1], int(parts[2])))
            if parts[0] == "rollback":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.deploy rollback <name> [target_revision]")
                target_revision = int(parts[2]) if len(parts) > 2 else None
                return self._json_command_result(runtime.rollback_deployment(parts[1], target_revision))
            return CommandResult(output="", error="usage: ns.deploy [status [name]] | ns.deploy rollout ... | ns.deploy promote ... | ns.deploy rollback ... | ns.deploy health ... | ns.deploy evaluate ...")
        except Exception as exc:
            return CommandResult(output="", error=str(exc))

    def _ns_recover(self, payload: str, _: str, __: Any) -> CommandResult:
        try:
            runtime = self._require_declarative_access("ns.recover", admin=False)
            parts = split_command(payload)
            if not parts or parts[0] == "list":
                return self._json_command_result(
                    {
                        "playbooks": runtime.list_recovery_playbooks(),
                        "runs": runtime.context.cluster.list_recovery_runs(),
                    }
                )
            runtime = self._require_declarative_access("ns.recover.mutate", admin=True)
            if parts[0] == "plan":
                if len(parts) < 3:
                    return CommandResult(output="", error="usage: ns.recover plan <name> <snapshot_path> [steps_json]")
                steps = None
                if len(parts) > 3:
                    parsed = self._parse_json_argument(parts[3])
                    if not isinstance(parsed, list):
                        return CommandResult(output="", error="recovery steps must be a JSON array")
                    steps = parsed
                return self._json_command_result(runtime.register_recovery_playbook(parts[1], self._resolve_path(parts[2]), steps=steps))
            if parts[0] == "run":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.recover run <name>")
                payload = runtime.run_recovery_playbook(parts[1])
                self._register_declarative_mesh_workers(runtime)
                return self._json_command_result(payload)
            return CommandResult(output="", error="usage: ns.recover [list] | ns.recover plan ... | ns.recover run ...")
        except Exception as exc:
            return CommandResult(output="", error=str(exc))

    def _ns_control(self, payload: str, _: str, __: Any) -> CommandResult:
        try:
            runtime = self._require_declarative_access("ns.control", admin=False)
            parts = split_command(payload)
            if not parts or parts[0] == "status":
                return self._json_command_result(runtime.control_status())

            if parts[0] == "queue":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.control queue <enqueue|list|run> ...")
                if parts[1] == "enqueue":
                    runtime = self._require_declarative_access("ns.control.queue.enqueue", admin=True)
                    if len(parts) < 3:
                        return CommandResult(output="", error="usage: ns.control queue enqueue <flow> [payload_json_or_path] [priority] [queue] [max_attempts] [base_backoff_seconds]")
                    payload_value = self._parse_json_argument(parts[3]) if len(parts) > 3 else None
                    priority = int(parts[4]) if len(parts) > 4 else 100
                    queue_name = parts[5] if len(parts) > 5 else "default"
                    max_attempts = int(parts[6]) if len(parts) > 6 else 3
                    base_backoff_seconds = float(parts[7]) if len(parts) > 7 else 5.0
                    return self._json_command_result(
                ...
                if parts[1] == "list":
                    queue_name = parts[2] if len(parts) > 2 else None
                    status = parts[3] if len(parts) > 3 else None
                    return self._json_command_result({"tasks": runtime.list_queue_tasks(queue_name=queue_name, status=status)})
                if parts[1] == "run":
                    runtime = self._require_declarative_access("ns.control.queue.run", admin=True)
                    queue_name = parts[2] if len(parts) > 2 else None
                    limit = int(parts[3]) if len(parts) > 3 else 10
                    return self._json_command_result(runtime.run_pending_tasks(queue_name=queue_name, limit=limit))
                return CommandResult(output="", error="usage: ns.control queue <enqueue|list|run> ...")

            if parts[0] == "schedule":
                runtime = self._require_declarative_access("ns.control.schedule", admin=True)
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.control schedule <add-flow|add-event|list|tick> ...")
                if parts[1] == "add-flow":
                    if len(parts) < 5:
                        return CommandResult(output="", error="usage: ns.control schedule add-flow <job> <flow> <interval_seconds> [payload_json_or_path] [queue]")
                    payload_value = self._parse_json_argument(parts[5]) if len(parts) > 5 else None
                    queue_name = parts[6] if len(parts) > 6 else "default"
                    return self._json_command_result(runtime.schedule_flow(parts[2], parts[3], interval_seconds=float(parts[4]), payload=payload_value, queue_name=queue_name))
                if parts[1] == "add-event":
                    if len(parts) < 5:
                        return CommandResult(output="", error="usage: ns.control schedule add-event <job> <event> <interval_seconds> [payload_json_or_path] [queue]")
                    payload_value = self._parse_json_argument(parts[5]) if len(parts) > 5 else None
                    queue_name = parts[6] if len(parts) > 6 else "default"
                    return self._json_command_result(runtime.schedule_event(parts[2], parts[3], interval_seconds=float(parts[4]), payload=payload_value, queue_name=queue_name))
                if parts[1] == "list":
                    return self._json_command_result({"schedules": runtime.list_schedules()})
                if parts[1] == "tick":
                    return self._json_command_result(runtime.scheduler_tick())
                return CommandResult(output="", error="usage: ns.control schedule <add-flow|add-event|list|tick> ...")

            if parts[0] == "daemon":
                runtime = self._require_declarative_access("ns.control.daemon", admin=True)
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.control daemon <start|stop|tick|status> ...")
                if parts[1] == "start":
                    interval_seconds = float(parts[2]) if len(parts) > 2 else 1.0
                    task_limit = int(parts[3]) if len(parts) > 3 else 4
                    return self._json_command_result(runtime.start_control_daemon(interval_seconds=interval_seconds, task_limit=task_limit))
                if parts[1] == "stop":
                    return self._json_command_result(runtime.stop_control_daemon())
                if parts[1] == "tick":
                    task_limit = int(parts[2]) if len(parts) > 2 else 4
                    return self._json_command_result(runtime.control_tick(task_limit=task_limit))
                if parts[1] == "status":
                    return self._json_command_result(runtime.control_status().get("daemon", {}))
                return CommandResult(output="", error="usage: ns.control daemon <start|stop|tick|status> ...")

            if parts[0] == "api":
                runtime = self._require_declarative_access("ns.control.api", admin=True)
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.control api <start|stop|status> [host] [port] [token]")
                if parts[1] == "start":
                    host = parts[2] if len(parts) > 2 else "127.0.0.1"
                    port = int(parts[3]) if len(parts) > 3 else 8781
                    token = parts[4] if len(parts) > 4 and parts[4] != "-" else None
                    return self._json_command_result(runtime.start_control_api(host=host, port=port, auth_token=token))
                if parts[1] == "stop":
                    return self._json_command_result(runtime.stop_control_api())
                if parts[1] == "status":
                    return self._json_command_result(runtime.control_api_status())
                return CommandResult(output="", error="usage: ns.control api <start|stop|status> [host] [port] [token]")

            if parts[0] == "events":
                event_name = parts[1] if len(parts) > 1 and not parts[1].isdigit() else None
                since_index = 2 if event_name is not None else 1
                since_sequence = int(parts[since_index]) if len(parts) > since_index else 0
                limit = int(parts[since_index + 1]) if len(parts) > since_index + 1 else 50
                return self._json_command_result({"events": runtime.replay_event_log(event_name=event_name, since_sequence=since_sequence, limit=limit)})

            if parts[0] == "state":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.control state <list|get|replay> ...")
                if parts[1] == "list":
                    tenant_id = parts[2] if len(parts) > 2 and parts[2] != "-" else None
                    namespace = parts[3] if len(parts) > 3 and parts[3] != "-" else None
                    return self._json_command_result({"states": runtime.list_state(tenant_id=tenant_id, namespace=namespace)})
                if parts[1] == "get":
                    if len(parts) < 3:
                        return CommandResult(output="", error="usage: ns.control state get <key>")
                    states = runtime.list_state(tenant_id=runtime.context.active_tenant, namespace=runtime.context.active_namespace, limit=1000)
                    row = next((item for item in states if item.get("key") == parts[2]), None)
                    return self._json_command_result({"state": row})
                if parts[1] == "replay":
                    since_sequence = int(parts[2]) if len(parts) > 2 else 0
                    key = parts[3] if len(parts) > 3 and parts[3] != "-" else None
                    return self._json_command_result({"changes": runtime.replay_state_log(since_sequence=since_sequence, key=key)})
                return CommandResult(output="", error="usage: ns.control state <list|get|replay> ...")

            if parts[0] == "replica":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.control replica <peer|sync|records> ...")
                if parts[1] == "peer":
                    if len(parts) < 3:
                        return CommandResult(output="", error="usage: ns.control replica peer <list|add> ...")
                    if parts[2] == "list":
                        return self._json_command_result({"peers": runtime.list_replica_peers()})
                    if parts[2] == "add":
                        runtime = self._require_declarative_access("ns.control.replica.peer.add", admin=True)
                        if len(parts) < 5:
                            return CommandResult(output="", error="usage: ns.control replica peer add <name> <endpoint> [token] [tls_profile]")
                        token = parts[5] if len(parts) > 5 and parts[5] != "-" else None
                        tls_profile = parts[6] if len(parts) > 6 and parts[6] != "-" else None
                        return self._json_command_result(runtime.register_replica_peer(parts[3], parts[4], auth_token=token, tls_profile=tls_profile))
                    return CommandResult(output="", error="usage: ns.control replica peer <list|add> ...")
                if parts[1] == "sync":
                    runtime = self._require_declarative_access("ns.control.replica.sync", admin=True)
                    limit = int(parts[2]) if len(parts) > 2 else 100
                    return self._json_command_result(runtime.sync_replication(limit=limit))
                if parts[1] == "records":
                    since_sequence = int(parts[2]) if len(parts) > 2 else 0
                    record_type = parts[3] if len(parts) > 3 and parts[3] != "-" else None
                    return self._json_command_result({"records": runtime.list_replicated_records(since_sequence=since_sequence, record_type=record_type)})
                return CommandResult(output="", error="usage: ns.control replica <peer|sync|records> ...")

            if parts[0] == "workflow":
                if len(parts) < 2:
                    return CommandResult(output="", error="usage: ns.control workflow <list|replay> ...")
                if parts[1] == "list":
                    flow_name = parts[2] if len(parts) > 2 and parts[2] != "-" else None
                    return self._json_command_result({"runs": runtime.list_workflow_runs(flow_name=flow_name)})
                if parts[1] == "replay":
                    runtime = self._require_declarative_access("ns.control.workflow.replay", admin=True)
                    if len(parts) < 3:
                        return CommandResult(output="", error="usage: ns.control workflow replay <run_id>")
                    return self._json_command_result(runtime.replay_workflow_run(parts[2]))
                return CommandResult(output="", error="usage: ns.control workflow <list|replay> ...")

            if parts[0] == "metrics":
                format_name = parts[1] if len(parts) > 1 else "json"
                payload_value = runtime.export_metrics(format_name)
                if isinstance(payload_value, str):
                    return CommandResult(output=payload_value if payload_value.endswith("\n") else payload_value + "\n", data=payload_value, data_type=PipelineType.TEXT)
                return self._json_command_result(payload_value)

            if parts[0] == "services":
                if len(parts) < 2 or parts[1] == "list":
                    return self._json_command_result({"services": runtime.list_services()})
                if parts[1] == "deploy":
                    runtime = self._require_declarative_access("ns.control.services.deploy", admin=True)
                    if len(parts) < 3:
                        return CommandResult(output="", error="usage: ns.control services deploy <service_name>")
                    return self._json_command_result(runtime.deploy_service(parts[2]))
                return CommandResult(output="", error="usage: ns.control services [list] | ns.control services deploy <service_name>")

            if parts[0] == "packages":
                if len(parts) < 2 or parts[1] == "list":
                    return self._json_command_result({"packages": runtime.list_packages()})
                if parts[1] == "install":
                    runtime = self._require_declarative_access("ns.control.packages.install", admin=True)
                    if len(parts) < 3:
                        return CommandResult(output="", error="usage: ns.control packages install <package_name>")
                    return self._json_command_result(runtime.install_package(parts[2]))
                return CommandResult(output="", error="usage: ns.control packages [list] | ns.control packages install <package_name>")

            return CommandResult(output="", error="usage: ns.control [status] | ns.control queue ... | ns.control schedule ... | ns.control daemon ... | ns.control api ... | ns.control events ... | ns.control state ... | ns.control replica ... | ns.control workflow ... | ns.control metrics ... | ns.control services ... | ns.control packages ...")
        except Exception as exc:
            return CommandResult(output="", error=str(exc))

    def _ns_snapshot(self, payload: str, _: str, __: Any) -> CommandResult:
        if self._declarative_nova is None:
            return CommandResult(output="", error="no active declarative Nova runtime")
        parts = split_command(payload)
        target = parts[0] if parts else ""
        try:
            snapshot = self._declarative_nova.snapshot(self._resolve_path(target) if target else None)
            return CommandResult(output=json.dumps(snapshot, ensure_ascii=False) + "\n", data=snapshot, data_type=PipelineType.OBJECT)
        except Exception as exc:
            return CommandResult(output="", error=str(exc))

    def _ns_resume(self, payload: str, _: str, __: Any) -> CommandResult:
        parts = split_command(payload)
        if not parts:
            return CommandResult(output="", error="usage: ns.resume <snapshot.json>")
        try:
            if self._declarative_nova is not None:
                with contextlib.suppress(Exception):
                    self._declarative_nova.close()
            ...
            result = runtime.resume(self._resolve_path(parts[0]))
            self._register_declarative_mesh_workers(runtime)
            self._declarative_nova = runtime
            self._ns_runtime = None
    ...
    def _ns_check(self, source: str, _: str, __: Any) -> CommandResult:
        ...
        try:
            ...
            if self._is_declarative_nova_source(source_text):
                program = DeclarativeNovaRuntime().compile(source_text, source_name=str(target), base_path=target.parent)
                ast = program.ast
                result = {
                    "declarations": len(ast.declarations),
                    "agents": len(ast.agents()),
                    "datasets": len(ast.datasets()),
                    "tools": len(ast.tools()),
                    "flows": len(ast.flows()),
                    "events": len(ast.events()),
                    "states": len(ast.states()),
                    "systems": len(ast.systems()),
    ...
    def _local_worker_command(
        ...
        *,
        ...
        if getattr(sys, "frozen", False):
            command = [sys.executable, "--serve-worker", "--worker-host", host, "--worker-port", str(port), "--worker-caps", caps_text]
        else:
            command = [sys.executable, str(Path(__file__).resolve()), "--serve-worker", "--worker-host", host, "--worker-port", str(port), "--worker-caps", caps_text]
        if auth_token:
            command.extend(["--worker-token", auth_token])
        if certfile:
            command.extend(["--worker-cert", certfile])
        if keyfile:
            command.extend(["--worker-key", keyfile])
        if cafile:
    ...
    def _wait_for_worker_health(self, url: str, *, timeout_seconds: float = 10.0, cafile: str | None = None) -> bool:
        ...
        while time.time() < deadline:
            try:
                open_kwargs: dict[str, Any] = {"timeout": 1}
                ...
                with urllib.request.urlopen(url.rstrip("/") + "/health", **open_kwargs) as response:
        ...
        if action == "start-worker":
            ...
            while i < len(parts):
                ...
                if token == "--caps" and i + 1 < len(parts):
                    ...
                    i += 2
                    continue
                if token == "--labels" and i + 1 < len(parts):
                    labels = self._parse_key_value_pairs(parts[i + 1])
                    i += 2
                    continue
                if token == "--tenant" and i + 1 < len(parts):
                    tenant = parts[i + 1]
                    i += 2
                    continue
                if token == "--token" and i + 1 < len(parts):
                    auth_token = parts[i + 1]
                    i += 2
                    continue
                if token == "--tls-cert" and i + 1 < len(parts):
                    certfile = str(self._resolve_path(parts[i + 1]))
                    i += 2
                    continue
                if token == "--tls-key" and i + 1 < len(parts):
                    keyfile = str(self._resolve_path(parts[i + 1]))
                    i += 2
                    continue
                if token == "--tls-ca" and i + 1 < len(parts):
                    cafile = str(self._resolve_path(parts[i + 1]))
                    i += 2
                    continue
                i += 1
            if port <= 0:
            ...
            if certfile and keyfile:
                tls_profile = f"mesh-worker-{uuid.uuid4().hex[:8]}"
            ...
            self.mesh.add_worker(url, caps, labels=labels, tenant=tenant, auth_token=auth_token, tls_profile=tls_profile)
        ...
        if action == "add":
            ...
            while i < len(parts):
                ...
                if token == "--token" and i + 1 < len(parts):
                    ...
                    i += 2
                    continue
                if token == "--tls-profile" and i + 1 < len(parts):
                    tls_profile = parts[i + 1]
                    i += 2
                    continue
                i += 1
            self.mesh.add_worker(parts[1], caps, labels=labels, tenant=tenant, auth_token=auth_token, tls_profile=tls_profile)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--worker-token", default="", help="bearer token required by --serve-worker")
    ...
    if args.serve_worker:
        ...
        worker_server = MeshWorkerServer(
            shell,
            caps,
            auth_token=args.worker_token or None,
            certfile=args.worker_cert or None,
            keyfile=args.worker_key or None,
